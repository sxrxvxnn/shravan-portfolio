<script lang="ts">
	...
	onMount(() => {
		...
		if (!rawCtx) { done = true; onDone(); return; }
		const ctx = rawCtx;
		...
		// Emergency: if something hangs, still show site
		const emergency = setTimeout(fireOnDone, 6500);
			...
			try {
				...
				if (now > T_LETTERS) {
					...
					if (letterIdx > 0) {
						...
						try { (ctx as any).letterSpacing = `${FONT_SIZE * 0.08}px`; } catch {}
				...
				if (!doneFired) frame = requestAnimationFrame(draw);
			} catch {
				// If canvas crashes, still show site
				clearTimeout(emergency);
				fireOnDone();
			}
		}

		// Start draw — fallback to setTimeout if fonts.ready hangs
